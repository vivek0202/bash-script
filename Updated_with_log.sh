...
while read -r SERVICE; do
  ...
  # Find log files matching all 3 patterns
  ...
  for LOG in $MATCHED_LOGS; do
    if grep -q "$ERROR_PATTERN" "$LOG"; then
      ...
      break  # Only restart once per service
    fi
  done

done < "$SERVICES_FILE"

# === Send email if any restart occurred ===
if grep -q "Restarted" "$TEMP_REPORT"; then
  mail -s "Service Restart Alert - Connection Reset Detected" "$EMAIL" < "$TEMP_REPORT"
fi
